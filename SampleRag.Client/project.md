...
- **API-слой:** В коде отсутствует. По спецификации все запросы к локальному RAG API; аутентификация — Bearer JWT, access token в памяти, refresh в HttpOnly cookie; при 401 — тихий refresh без retry 401/403.
...
Назначение: сборка и dev-сервер. Задаёт порт 61413 и React-плагин.
...
- **Переиспользуемые паттерны:** Кастомный TokenManager (401 → refresh → retry), react-dropzone для валидации PDF (тип, 20MB) до отправки на сервер, FSD для масштабирования фич.
...
- **Скрипты (package.json):**  
  - `dev` — `vite` (запуск dev-сервера на порту 61413).  
